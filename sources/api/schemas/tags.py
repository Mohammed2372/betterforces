...
class WeakTagsResponse(BaseModel):
    ...
    weak_tags: List[SimpleTagInfoSchema] = Field(..., description="List of weak tags (significantly lower average rating)")
    overall_average_rating: float = Field(..., description="Overall average rating of all solved problems")
    total_solved: int = Field(..., description="Total number of problems solved")
    threshold_used: int = Field(..., description="Rating difference threshold used to identify weak tags")
    last_updated: datetime = Field(..., description="Timestamp when data was last fetched")
